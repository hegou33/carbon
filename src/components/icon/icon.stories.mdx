...
If using the `bgSize` prop in combination with `fontSize`, the `fontSize` prop value will take precedence and the background size may be adjusted to accommodate for a larger icon. 
